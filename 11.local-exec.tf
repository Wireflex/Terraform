# запуск локальных команд, то есть создавать ресурсы не будет(только файлы, если укажем, как names.txt,log.txt к примеру) и выводить всякие ping,print итд в терминале
