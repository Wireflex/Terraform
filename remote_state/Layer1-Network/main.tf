...
# но может возникнуть такая ситуация, что 2 разраба запушат изменений одновременно и будут ошибки, либо ваще чёт сломается, в таком случае можно юзать DynamoDB в AWS, добавив `dynamodb_table` в блок backend"s3", если нет достаточных пермишинов - приатачиваем DynanoDB Table Permission
# и при terraform apply ( но пока без подтверждения "yes" !!! ) в DynamoDB создаётся temp директория tfstate, это состояние блокируется и 2ой разраб уже не сможет сделать terraform apply
