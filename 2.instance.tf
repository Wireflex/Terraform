...
Lifecycles
используется для того, чтобы обеспечить экономически эффективное хранение объектов на протяжении всего их жизненного цикла
  ...
  lifecycle {
    ignore_changes = ["ami", "user_data"]     # если этот параметр заставляет терраформ сделать дестрой - не делать этого (просто проигнорит изменения)
  }
